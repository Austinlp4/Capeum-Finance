    ...
    const handleContinue = () => {
        ...
        const mappedData = {
            headers: headers.map((_header, index) => {
                const columnIndex = getColumnIndex(`column-${index}`);
                return selectedColumns[`column-${columnIndex}`] || null;
            }),
            body: body.map((row) => {
                const transformedRow = row.map((cell, index) => {
                    const columnIndex = getColumnIndex(`column-${index}`);
                    return selectedColumns[`column-${columnIndex}`] ? cell : null;
                });
                ...
                <CardHeader className="gap-y-2 lg:flex-row lg:items-center lg:justify-between">
                    ...
                    <div className="flex flex-col lg:flex-row items-center gap-x-2 gap-y-2">
                        <Button 
                            size={'sm'} 
                            onClick={onCancel} 
                            className="bg-slate-300 hover:bg-slate-300/80 text-muted-foreground w-full lg:w-auto"
                        >
                            Cancel
                        </Button>
                        <Button
